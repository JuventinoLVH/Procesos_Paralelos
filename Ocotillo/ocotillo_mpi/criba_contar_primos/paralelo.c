...
int main (int argc, char *argv[]){
    ...
    int primo_anterior,primer_elemento_primo=-1,siguiente_es_primo;
    int primos_cont=0;    
    int primos=0;
  ...
  ###########################################################################*/
    ...
    */
    if(id != p-1 )
    {
        ...
        primos = (primes[iterador_local-1] && siguiente_es_primo)? primos+1 : primos ;
    }
    
    // Reduce de cajon
    MPI_Reduce(&primos_cont, &primos, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD); 
/*#############################################################################
