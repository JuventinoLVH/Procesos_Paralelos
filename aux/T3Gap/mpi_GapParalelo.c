...
int main (int argc, char *argv[]){
    ...
    int prime = 0, firstPrime = 0, lastPrime = 0, localGap = 0, firstReceived;
    for (j = 0; j < size; j++){
            if (primes[j]) {
               prime_count++;
                if (!prime){
                ...
                if((2*j+ 3) - (2*prime + 3) > localGap){
                    localGap = (2*j+ 3) - (2*prime + 3);
                }
                prime = j;
                lastPrime = j;
            }
    ...
    if( prime_count == 0 ){
      printf("Hay demasiados procesos %d\n",id);
      MPI_Finalize();
      return 1;
    }
    ...
    if(id != 0 ){
      //printf("id: %d, send: %d \n", id, firstPrime);
      MPI_Send(&firstPrime, 1, MPI_INT, id-1, 9, MPI_COMM_WORLD);
    }

    if(id != p-1){
      MPI_Recv(&firstReceived, 1, MPI_INT, id+1, 9, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      if(firstPrime == 0 && id != 0) MPI_Send(&firstReceived, 1, MPI_INT, id-1, 9, MPI_COMM_WORLD);
      if (firstReceived != 0 && lastPrime != 0 && (2*firstReceived+3)-(2*lastPrime+3)-1 > localGap) 
        localGap = (2*firstReceived+3)-(2*lastPrime+3);
    }

    //printf("id: %d, Rec: %d \n", id, firstReceived);
    //printf("id: %d,   ini: %d,   fin: %d,   gap: %d \n", id, low_proc_value, high_proc_value,localGap);
    
    MPI_Reduce (&localGap, &global_count, 1, MPI_INT, MPI_MAX, 0, MPI_COMM_WORLD);    
    ...
    if (!id) {
        //printf ("El número de primos es %d\n", global_count);
